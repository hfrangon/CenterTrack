...
from src.lib.model.networks.transformer.Models import Encoder_patch66
...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
        ...
        if opt.pre_img:
            self.pre_img_layer = nn.Sequential(
                ...
                nn.BatchNorm2d(channels[0], momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True))
        self.transformer = Encoder_patch66(d_model=256, d_inner=1024, n_layers=2)
        if opt.pre_hm:
    ...
    def forward(self, x, pre_img=None, pre_hm=None):
        ...
        x_unfold = F.unfold(x, kernel_size=4, dilation=1, stride=4, padding=0)
        x_unfold = x_unfold.permute(0, 2, 1)

        mask_unfold = F.unfold(pre_hm, kernel_size=4, dilation=1, stride=4, padding=0)
        mask_unfold = mask_unfold.permute(0, 2, 1)
        mask_unfold = torch.mean(mask_unfold, dim=2).unsqueeze(dim=-2)
        mask_unfold[mask_unfold <= 0.3] = 0.0

        x_unfold = self.transformer(x_unfold, src_mask=mask_unfold)
        x_unfold = x_unfold.permute(0, 2, 1)
        x = nn.Fold(output_size=(height, width), kernel_size=(4, 4), stride=4, padding=0, dilation=1)(
            x_unfold)
